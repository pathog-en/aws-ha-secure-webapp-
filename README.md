...
High level overview:
  ...
  Traffic flow
    user traffic enters via ALB over HTTPS -> ALB forwards reqs to compute resources in private     subnets -> app tier communicates with database tier internally -> outbound traffic from         private subnets flows through the NAT gw only
  compute layer
